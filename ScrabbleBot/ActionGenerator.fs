            ...
            fun acc letterIndex _  ->
                ...
                Set.fold (
                    fun _ elem -> 
                        ...
                        | _, false -> acc
                        | None, _ -> acc //No move
                        | Some(false, dic), _ -> aux (MultiSet.removeSingle letterIndex hand) dic (placement::move) acc tiles (incrementCoord isHorizontal (x, y) )
                        | Some(true, _), _ -> (placement::move)::acc
                ) acc tile
        )moveList hand
